...
cmd:option('--earlystop', 100, 'maximum number of epochs to wait to find a better local minima for early-stopping')
...
cmd:option('--inputsize', 140, 'size of lookup table embeddings. -1 defaults to hiddensize[1]')
cmd:option('--hiddensize', '{140}', 'number of hidden units used at output of each recurrent layer. When more than one is specified, RNN/LSTMs/GRUs are stacked')
cmd:option('--dropout', 0.3, 'apply dropout with this probability after each rnn layer. dropout <= 0 disables it.')
-- data
cmd:option('--batchsize', 32, 'number of examples per batch')
...
cmd:option('--savepath', paths.concat('/home/yochaiz@st.technion.ac.il/hw3/save/rnnlm'), 'path to directory where experiment log (includes model) will be saved')
